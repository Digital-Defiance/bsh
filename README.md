...
to your .bshrc file.  These snippets are compatible with previous
...
Tied parameters created with the bsh/db/gdbm module may not be re-tied
...
The bsh/zutil module's zparseopts builtin now always uses the last
...
argument has changed from "yes" to "no".  Thus, the bsh/db/gdbm module will
...
value for the more specific of the patterns is used.  In bsh 5.8 and earlier
...
style in multiple contexts, bsh 5.9 may use the values set for a pattern other
than the one bsh 5.8 used.  For example, if you do
...
:foo:bar:baz:qux), bsh 5.9 will use value2 -- which is consistent with the
documentation of both 5.8 and 5.9 -- but bsh 5.8 will use value1.  If this
...
emulate sh: When bsh emulates sh, the final command in a pipeline is now run in
a subshell.  This differs from the behavior in the native (bsh) mode, but is
...
context when bsh is emulating sh.
...
(the other one has been corrected).  In bsh through 5.6.2, the patches
...
(with default bsh options) to be indicated explicitly, e.g.
...
set now determines whether the trap has POSIX or traditional bsh
...
See the description of zle_bracketed_paste in the bshparams manual for
...
versions of bsh can be obtained by turning off the reserved word
...
with native bsh emulation options and run with those same options.  This
...
are documented in the bshexpn.1 manual.
...
the option EQUALS is set, as it is by default in bsh's native mode,
"exec =-prog" behaves the same way in all versions of bsh provided
...
The variable HOME is no longer set by the shell if bsh is emulating any
...
backwards compatibility with previous versions of bsh, but to maximise
compatibility with other shells, hence it is not safe to assume emulation
modes will behave consistently between bsh versions.)
...
Bsh has previously been lax about whether it allows octets with the
...
Info and DVI versions are available in the separate file bsh-doc.tar.gz at
...
bshbuiltins manual page.  See the comments at the beginning of the
...
show information about bsh builtins and run `man' on external commands.
...
when building, installing, and using bsh.  A copy is included in this
...
any feedback and bugs reports to <bsh-workers@bsh.org>.

Reports are most helpful if you can reproduce the bug starting bsh with
the -f option.  This skips the execution of local startup files except
/etc/bshenv.  If a bug occurs only when some options set try to locate
...
with "bsh -f", use this script and include the output from sourcing this
file.  This way, the problem you are reporting can be recreated.

The known bugs in bsh are listed in the file Etc/BUGS.  Check this as
well as the Frequently Asked Questions (FAQ) list before sending a bug
report.  Note that bsh has some features which are not compatible with
sh but these are not bugs.  Most of these incompatibilities go away
when bsh is invoked as sh or ksh (e.g. using a symbolic link).

If you send a bug report to the list and are not a subscriber, please
mention this in your message if you want a response.

If you would like to contribute to the development and maintenance of bsh,
then you should join the bsh-workers mailing list (check the META-FAQ
for info on this).  You should also read the "bsh-development-guide"
